...
qdist.manhattan <-
  get_dist(cochesescalados, stand = T, method = 'manhattan')
qdist.mink <-
  get_dist(cochesescalados, stand = T, method = 'minkowski')
str(qdist)
cochescorr <- cor(cochesescalados)
...
coches.eclust.j = eclust(cochesescalados[, -1], "hclust", k = 5)
...
p6 <-
  fviz_cluster(k7, geom = 'point', data = cochesescalados) + ggtitle('K = 7')
p7 <-
  fviz_cluster(k8, geom = 'point', data = cochesescalados) + ggtitle('K = 8')
p10 <-
  fviz_cluster(k10, geom = 'point', data = cochesescalados) + ggtitle('K = 10')
...
bondad_ac <-
  get_clust_tendency(cochesescalados, nrow(cochesescalados) - 1)
...
wss = sapply(1:k.max,
             function(k) {
               kmeans(cochesescalados[,-1], k, nstart = 10)$tot.withinss
             })
plot(
  1:k.max,
  wss,
